...
class LoginController extends Controller {
    ...
    public function actionIndex() {
        ...
        $code = yii::$app->session->get('__captcha/admin/adminuser/login/captcha');
        //获取生成的验证码 与用户输入进行比较
...
//        }
        ...
        if (!isset($params['password'])) {
            return result(500, '缺少参数 password');
        }
        if (!isset($params['username'])) {
            return result(500, '缺少参数 username');
        }
        $where = [
            'username' => $params['username'],
            'delete_time is null' => null
        ];
        $res = $table->tableSingle('admin_user', $where);
        ...
        $tokenClass = new Token(yii::$app->params['JWT_KEY_ADMIN']);
        try {
