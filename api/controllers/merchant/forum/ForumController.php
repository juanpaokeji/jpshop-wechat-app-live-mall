 ...
 * @return array
 */
class ForumController extends MerchantController {
...
//    }
    ...
    public function actionList() {
        if (yii::$app->request->isGet) {
            ...
            $array = $forumModel->findall($params);
            return $array;
        } else {
            return result(500, "请求方式错误");
        }
    ...
    public function actionConfig() {
        if (yii::$app->request->isPut) {
            ...
            if ($array['status'] == 200) {
                $stauts = json_decode($array['data']['config'], true);
                $params['must_keyword'] = isset($params['must_keyword']) ? $params['must_keyword'] : $stauts['must_keyword'];
                $params['must_examine'] = isset($params['must_examine']) ? $params['must_examine'] : $stauts['must_examine'];
                $params['allow_post_time'] = isset($params['allow_post_time']) ? $params['allow_post_time'] : $stauts['allow_post_time'];
                $params['allow_comment_level'] = isset($params['allow_comment_level']) ? $params['allow_comment_level'] : $stauts['allow_comment_level'];
                $params['illegally'] = isset($params['illegally']) ? $params['illegally'] : $stauts['illegally'];
                $params['score'] = isset($params['score']) ? $params['score'] : $stauts['score'];
...
//                }
                ...
                $params['config'] = json_encode($params, JSON_UNESCAPED_UNICODE);
                ...
                unset($params['score']);
                $forum = $forumModel->update($params);
                return $forum;
            } else {
                return result(204, "查询失败");
            }
