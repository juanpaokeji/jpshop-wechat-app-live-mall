 ...
 * @Bean()
 */
class AppModel extends TableModel {
    ...
    public function findall($params) {
        ...
        for ($i = 0; $i < count($app); $i++) {
           ...
           // $rs = $table->tableSingle('system_app_category', ['id' => $app[$i]['category_id']]);
    ...
    public function find($params) {
        ...
        //数据库操作
        try {
            $app = $table->tableSingle('system_app', ['id' => $params['id'], 'delete_time is null' => null]);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
    ...
    public function add($params) {
        ...
        $res = $table->tableAdd('system_app', $params);
        try {
    ...
    public function delete($params) {
        ...
        $table = new TableModel();
        try {
            $res = $table->tableUpdate('system_app', $params, $where);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
    ...
    public function update($params) {
        ...
        $table = new TableModel();
            ...
            $res = $table->tableUpdate('system_app', $params, $where);try {
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
