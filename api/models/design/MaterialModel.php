 ...
 * @Bean()
 */
class MaterialModel extends TableModel {

    public $table = "design_material";
    ...
    public function findall($params) {
        ...
        if (empty($app)) {
            ...
            if ($params['type'] != 4) {
                $num = floor($res['count'] / $params['limit']) + 1;
                return ['status' => 200, 'message' => '请求成功', 'data' => $app, 'count' => $num];
            } else {
     ...
     * @return array
     */
    public function findalls($params) {
        //数据库操作
        $table = new TableModel();
        try {
            $params['delete_time is null'] = null;
        ...
        $str = "";
        for ($i = 0; $i < count($app); $i++) {
            $str .= "@font-face{font-family:'{$app[$i]['name']}';src:url({$app[$i]['pic_url']})}" . "\n";
        }
        if (empty($app)) {
            return result(204, '查询失败');
        } else {
            return ['status' => 200, 'message' => '请求成功', 'data' => $str];
        }
    }
