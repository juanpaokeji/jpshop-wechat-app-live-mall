...
class UserCollectionModel extends TableModel {
    ...
    public function finds($params) {
        ...
        try {
            ...
            $table = new TableModel();
            $sql = "select fuc.*,fu.nickname,fu.avatar,fu.sex,fp.address,fp.pic_urls,fp.content,fp.title,fp.voice_url,fk.name as keywords from forum_user_collection as fuc inner join forum_post as fp on fp.id = fuc.post_id INNER JOIN forum_user as fu on fu.id=fp.user_id  LEFT JOIN forum_keywords as fk  ON fk.id = fp.keywords_id where  fuc.delete_time is null and fuc.`key` ='{$params['`key`']}' and fuc.merchant_id = {$params['merchant_id']} and fuc.user_id = {$params['user_id']} and fuc.`status` =1 and fp.status =1 and fp.delete_time is null order by fuc.id desc" . $limit;
