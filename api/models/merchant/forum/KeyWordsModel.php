...
class KeyWordsModel extends TableModel {
    ...
    public function find($params) {
        ...
        //数据库操作
        try {
            $app = $table->tableSingle('forum_keywords', ['id' => $params['id'], 'delete_time is null' => null, 'status' => 1]);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
    ...
    public function add($params) {
        ...
        try {
            ...
            $res = $table->tableAdd('forum_keywords', $params);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
    ...
    public function delete($params) {
        ...
        $table = new TableModel();
        try {
            $res = $table->tableUpdate('forum_keywords', $params, $where);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
