...
class UserLikeModel extends TableModel {
    ...
    public function finds($params) {
        ...
        try {
            ...
            $table = new TableModel();
            $sql = "select ful.*,fu.nickname,fu.avatar,fu.sex,fp.address,fp.pic_urls,fp.content,fp.title,fp.voice_url from forum_user_like as ful INNER JOIN forum_user as fu on fu.id=ful.user_id  inner join forum_post as fp on fp.id = ful.source_id where  ful.delete_time is null and ful.`key` ='{$params['`key`']}' and ful.merchant_id = {$params['merchant_id']} and ful.user_id = {$params['user_id']} and ful.`status` =1 order by ful.id desc" . $limit;
