...
class UserModel extends TableModel {
        ...
        try {
            ...
            if (isset($params['startTime'])) {
                if ($params['startTime']) {
                    ...
                    $params["update_time >={$params['startTime']}"] = null;
                }
            ...
            if (isset($params['endTime'])) {
                if ($params['endTime']) {
                    ...
                    $params["update_time <={$params['endTime']}"] = null;
                }
    ...
    public function find($params) {
        ...
        //数据库操作
        try {
            $app = $table->tableSingle($this->table, ['id' => $params['id'], 'delete_time is null' => null, $params['merchant_id'], '`key`' => $params['`key`']]);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
    ...
    public function findAdmin($params) {

        $table = new TableModel();
        //数据库操作
        try {
            $app = $table->tableSingle($this->table, ['is_admin' => 9, 'delete_time is null' => null, 'merchant_id' => $params['merchant_id'], '`key`' => $params['`key`']]);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (gettype($app) != 'array') {
    ...
    public function updateAdmin($params) {
        //where 条件设置
        $where = ['is_admin' => 9, 'merchant_id' => $params['merchant_id'], '`key`' => $params['`key`'], 'id' => $params['id']];
        //params 参数值设置
        $params['update_time'] = time();
        //数据库操作
        $table = new TableModel();
