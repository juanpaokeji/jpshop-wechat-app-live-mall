...
class GroupModel extends TableModel {
    ...
    public function find($params) {
        ...
        //数据库操作
        try {
            $app = $table->tableSingle('shop_auth_group', ['id' => $params['id'], 'delete_time is null' => null]);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
    ...
    public function add($params) {
      ...
      /*  $rules = "";
        ...
        $data = [
            ...
            'is_kefu' => isset($params['is_kefu']) ? 1 : 0,
            'create_time' => time()
        ];
        //数据库操作
        $table = new TableModel();
        try {
            $res = $table->tableAdd('shop_auth_group', $data);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
    ...
    public function delete($params) {
        ...
        $table = new TableModel();
        try {
            $res = $table->tableUpdate('shop_auth_group', $data, $params);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
    ...
    public function update($params) {
      ...
      /*  if (isset($params['rules'])) {
        ...
        $table = new TableModel();
        try {
            $res = $table->tableUpdate('shop_auth_group', $params, $where);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
    ...
    public function rule($params) {
        ...
        $groups = $table->tableSingle('shop_auth_group', ["delete_time is null" => null, 'id' => $params['id']]);
        $str = "";
    ...
    public function users($params) {
        ...
        $table = new TableModel();
        try {
            $res = $table->tableList('shop_auth_group_access', $where, $fields);
        } catch (\Exception $e) {
            return result(500, '数据库操作失败');
        }
//        $res = $table->tableList('admin_user', $where, $fields);
        ...
        foreach ($res as $k => $v) {
            //循环获取 用户 名称
            try {
                $users = $table->tableSingle('system_sub_admin', ['id' => $v['uid']], 'username');
                $res[$k]['uid'] = $v['uid'];
