...
//            unset($app[$i]['name']);
    ...
    public function queryAll($params) {
        ...
        try {
            $table = new TableModel();
            $params['shop_user_cart.delete_time is null'] = null;
            $params['table'] = $this->table;
            $params['fields'] = " shop_user_cart.* ,sg.name as goods_name,shop_stock.weight,sg.supplier_id ";
            $params['join'] = " inner join shop_goods as sg on sg.id  = shop_user_cart.goods_id  inner join shop_stock on shop_stock.id = shop_user_cart.stock_id  ";
            $params['shop_user_cart.status'] = 1;
            $res = $table->tableList($params);
            $app = $res['app'];
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
    ...
    public function find($params) {
        ...
        if (isset($params['property1_name'])) {
            $where['property1_name'] = $params['property1_name'];
        }
        if (isset($params['property2_name'])) {
            $where['property2_name'] = $params['property2_name'];
        }
    ...
    public function delete($params) {
        ...
        $table = new TableModel();
            ...
            $res = $table->tableUpdate($this->table, $params, $where); try {
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (!$res) {
