 ...
 * @Bean()
 */
class DecorationModel extends TableModel {

    public $table = "system_merchant_design";
    ...
    public function isEnable($params) {
        ...
        $where_all = [];
        if (isset($params['`key`'])) {
            $where_all['`key`'] = $params['`key`'];
            $where['`key`'] = $params['`key`'];
            unset($params['`key`']);
        }
        if (isset($params['merchant_id'])) {
            $where_all['merchant_id'] = $params['merchant_id'];
            $where['merchant_id'] = $params['merchant_id'];
            unset($params['merchant_id']);
        }
        if (isset($params['id'])) {
        ...
        try {
            $table = new TableModel();
            $table->tableUpdate($this->table, ['is_enable'=>0], $where_all);
            $res = $table->tableUpdate($this->table, $params, $where);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
