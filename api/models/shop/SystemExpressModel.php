 ...
 * @Bean()
 */
class SystemExpressModel extends TableModel {

    public $table = "system_express";
    ...
    public function alls($params) {
        //数据库操作
        $table = new TableModel();
        $params['delete_time is null'] = null;
        $params['table'] = $this->table;
        $shopExpressParams = $params;
        unset($shopExpressParams['is_ok']);
        if (isset($params['searchName'])) {
            $params['searchName'] = trim($params['searchName']);
            $params["name like '%{$params['searchName']}%'"] = null;
            unset($params['searchName']);
        }
        $shopExpress = new ShopExpressModel();
        unset($shopExpressParams['searchName']);
        $shopExpressParams['table'] = "shop_express";
        $shopExpressData = $shopExpress->tableList($shopExpressParams);
        if (count($shopExpressData['app']) != 0) {
            $ids = "";
            for ($i = 0; $i < count($shopExpressData['app']); $i++) {
                if ($i == 0) {
                    $ids = $shopExpressData['app'][$i]['system_express_id'];
                } else {
                    $ids = $ids . "," . $shopExpressData['app'][$i]['system_express_id'];
                }
            }
            $params["id not in({$ids})"] = null;
        }
