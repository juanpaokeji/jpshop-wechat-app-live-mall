...
class VoucherModel extends TableModel {
    ...
    public function findall($params) {
        ...
        for ($i = 0; $i < count($app); $i++) {
            ...
            $rs1 = $table->tableSingle('shop_user', ['id' => $app[$i]['user_id'], 'delete_time is null' => null]);
            if ($rs1) {
                $app[$i]['user_id'] = $rs1['nickname'];
            } else {
                $app[$i]['user_id'] = "无";
            }
            $rs2 = $table->tableSingle('shop_voucher_type', ['id' => $app[$i]['type_id'], 'delete_time is null' => null]);
         //   $app[$i]['type_id'] = $rs2['name'];
            $app[$i]['type_name'] = $rs2['name'];
