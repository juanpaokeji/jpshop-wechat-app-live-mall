 ...
 * @Bean()
 */
class SystemHelpModel extends TableModel {

    public $table = "system_help";
    ...
    public function findall($params) {
        ...
        try {
            ...
            $params['system_help.delete_time is null'] = null;
            $params['table'] = $this->table;
            $params['fields'] = " system_help.*,system_help_category.name as category_name ";
            $params['orderby'] = " sort asc ";
            $params['join'] = " inner join system_help_category on system_help_category.id = system_help.category_id ";
     ...
     * @return array
     */
    public function lastFind($params) {
        $table = new TableModel();
        //数据库操作
        $orderBy = " id desc ";
        $where['delete_time is null'] = null;
        try {
            $app = $table->tableSingle($this->table, $where, "", $orderBy);
        } catch (Exception $ex) {
            return result(500, '数据库操作失败');
        }
        if (gettype($app) != 'array') {
