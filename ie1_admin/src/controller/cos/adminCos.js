...
layui.define(function (exports) {
    ...
    layui.use(['jquery', 'setter', 'admin', 'form'], function () {
        ...
        //页面不同属性
        var ajax_method = 'adminCos';//新ajax需要的参数 method
        ...
        if (res && res.data) {
            ...
            $("input[name=secretId]").val(res.data.secretId);
            $("input[name=secretKey]").val(res.data.secretKey);
            $("input[name=token]").val(res.data.token);
            $("input[name=Bucket]").val(res.data.Bucket);
            /*diy设置结束*/
        }
