...
layui.define(function (exports) {
    layui.use(['table', 'jquery', 'form', 'admin', 'setter'], function () {
        ...
        var cols = [//加载的表格
            ...
            {field: 'title', title: '权限标题', width: "25%"},
        ...
        form.on('submit(showAdd)', function () {
            ...
            if (!groupData) {
                ...
                var category = document.getElementById('pid');
        ...
        function getGroups(group_id) {
            $.ajax({
                url: baseUrl + '/rules?rule_type=1&status=1',
                type: "get",
                headers: headers,
                beforeSend: function () {
                    loading = layer.load(loadType, loadShade);//显示加载图标
                ...
                success: function (res) {
                    ...
                    for (var a = 0; a < res.data.length; a++) {
                        ...
                        if (group_id) {
                            ...
                            $('select[name=pid]').append("<option value=" + id + selected + ">" + name + "</option>");
                        } else {
                            $('select[name=pid]').append("<option value=" + id + ">" + name + "</option>");
                        }
                        form.render();
                    }
                    groupData = 1;
        ...
        form.on('select(searchType)', function (data) {
            var searchObj = {searchName: $("input[name=searchName]").val()};
            if (data.value !== '') {
                searchObj.type = data.value;
            }
            render.reload({
                where: searchObj,
                page: {curr: 1}
