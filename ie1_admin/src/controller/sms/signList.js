...
layui.define(function (exports) {
    layui.use(['table', 'jquery', 'form', 'admin', 'setter'], function () {
        ...
        //页面不同属性
        var url = baseUrl + "/signs";//当前页面主要使用 url
        var cols = [//加载的表格
            {field: 'name', title: '签名内容', width: "15%"},
            {field: 'remark', title: '备注', width: "15%"},
            {field: 'reply', title: '审批信息', width: "30%"},
            {field: 'auditStatus', title: '审核状态', width: "10%"},
            {field: 'status', title: '启用状态', templet: '#statusTpl', width: "10%"},
            {field: 'operations', title: '操作', toolbar: '#operations', width: "20%"}
        ];
        var filePut = '';

        //上传图片
        $("#addImg").change(function () {//加载图片至img
            ...
            if (window.FileReader) {
                ...
                reader.onloadend = function (e) {
                    ...
                    $("#DocumentScreenImg").attr("src", e.target.result);
                };
            }
            file = null;
        });
            ...
            if (layEvent === 'edit') {//修改
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        /*diy设置开始*/
                        $("input[name=text]").val(res.data.name);
                        $("input[name=remark]").val(res.data.remark);
                        $("#DocumentScreenImg").attr("src", res.data.pic_str);
