...
layui.define(function (exports) {
    layui.use(['table', 'jquery', 'form', 'admin', 'setter'], function () {
        ...
        //页面不同属性
        var url = baseUrl + "/vouchers";//当前页面主要使用 url
        var cols = [//加载的表格
            {field: 'cdkey', title: '抵用券码', width: '20%'},
            ...
            {field: 'type_name', title: '代金券类型', width: '10%'},
            {field: 'merchant_id', title: '商户', width: '10%'},
            {field: 'status', title: '状态', width: '5%', templet: '#statusTpl'},
            {field: 'operations', title: '操作', toolbar: '#operations', width: '10%'}
        ];
        var groupData = 0;//是否已加载抵用券类型 是 1 否 0
        var merchantData = 0;//是否已加载商户 是 1 否 0
        ...
        form.on('submit(showAdd)', function () {
            ...
            if (!merchantData) {
                ...
                var merchantCategory = document.getElementById('merchant_id');
                merchantCategory.options[0].selected = true;
            }
            ...
            if (layEvent === 'edit') {//修改
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (!merchantData) {
                            getMerchantGroups(res.data.merchant_id);
                        } else {
        ...
        /*动态添加单选框 商户*/
        function getMerchantGroups(group_id) {
            $.ajax({
                url: baseUrl + '/merchants?status=1',
                type: "get",
                headers: headers,
                beforeSend: function () {
                    loading1 = layer.load(loadType, loadShade);//显示加载图标
                },
                success: function (res) {
                    if (res.status == timeOutCode) {
                        layer.msg(timeOutMsg);
                    ...
                    for (var a = 0; a < res.data.length; a++) {
                        ...
                        if (group_id) {
                            ...
                            $('select[name=merchant_id]').append("<option value=" + id + selected + ">" + name + "</option>");
                        } else {
                            $('select[name=merchant_id]').append("<option value=" + id + ">" + name + "</option>");
                        }
                        form.render();
                    }
                    merchantData = 1;
