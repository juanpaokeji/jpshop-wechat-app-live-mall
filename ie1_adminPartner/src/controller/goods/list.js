...
layui.define(function (exports) {
    layui.use(['table', 'jquery', 'form', 'admin', 'setter', 'laypage', 'element'], function () {
        ...
        //页面不同属性
        var arr = {};
        var url = "partnerGoods";//当前页面主要使用 url
        ...
        $(document).on('click', '.showAdd', function () {
            ...
            //新增前查询是否有在使用的模板
            // $.ajax({
            //     url: baseUrl + '/merchantShopExpressTemplate?&status=1',
            //     type: 'get',
            //     async: false,
            //     headers: headers,
            //     beforeSend: function () {
        ...
        $(document).off('click', '.btn-switch').on('click', '.btn-switch', function () {
            ...
            $.ajax({
                url: baseUrl + "/partnerGood/" + $(this).data("id"),
                type: 'put',
                async: false,
        ...
        form.on('switch(is_topTpl)', function (obj) {
            ...
            $.ajax({
                url: baseUrl + "/partnerGood/" + this.value,
                type: 'put',
                async: false,
                data: {
                    is_top: is_topCode,
