...
layui.define(function (exports) {
    layui.use(['jquery', 'admin', 'setter', 'form'], function () {
        ...
        var payId = sessionStorage.getItem("packagePayId");//上个页面保存的支付应用id
        var payPrice = sessionStorage.getItem("packagePayPrice");//上个页面保存的支付应用价格
        $('.payPrice').html(payPrice);

        //支付宝支付点击事件
        $(document).off('click', '.aliPay').on('click', '.aliPay', function () {
            window.open(baseUrl + '/merchantPayCombo/' + payId + '?type=ali&key=' + saa_key + '&merchant_id=' + sessionStorage.getItem('merchant_id'), '_blank', 'width=800,height=700,menubar=no,toolbar=no,status=no,scrollbars=yes');
        ...
        $(document).off('click', '.wxPay').on('click', '.wxPay', function () {
            $.ajax({
                url: baseUrl + '/merchantPayCombo/' + payId + '?type=wechat&key=' + saa_key + '&merchant_id=' + sessionStorage.getItem('merchant_id'),
                type: "get",
                headers: headers,
                beforeSend: function () {
