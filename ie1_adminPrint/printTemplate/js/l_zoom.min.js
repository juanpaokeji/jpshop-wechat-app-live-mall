$.fn.extend({
  l_zoom: function(c) {
    ...
    var b = $(
      '<div class="zoom_right"></div><div class="zoom_rb"></div><div class="zoom_bottom"></div>'
    );
    this.append(b);
      ...
      window.onmousemove = function(l) {
        ...
        if (g.parent().attr("l_zoom_mode") === "free") {
          ...
          var j = (f + l.clientX - d.x) / f,
            k = (h + l.clientY - d.y) / h;
          if (j >= k) {
            g.parent().css({ width: j * f, height: j * h });
            if (parseInt(g.parent().css("height")) <= 5) {
              g.parent().css({ width: ((j * f) / (j * h)) * 5, height: 5 });
            }
          } else {
            g.parent().css({ width: k * f, height: k * h });
            if (parseInt(g.parent().css("width")) <= 5) {
              g.parent().css({ width: 5, height: ((k * h) / (k * f)) * 5 });
            }
          }
        }
      };
