 ...
 * Created DateTime: 应该创建于 2019/9/19
 * js 易联云
 */
...
layui.define(function (exports) {
    ...
    layui.use(['jquery', 'setter', 'admin', 'table', 'form'], function () {
        ...
        //页面不同属性
        var ajax_method = 'merchantPrints';//新ajax需要的参数 method
        var cols = [//加载的表格
            {field: 'name', title: '打印机名称'},
            {field: 'apikey', title: 'API密钥'},
            {field: 'machine_code', title: '打印机终端号'},
            {field: 'msign', title: '打印机终端密钥'},
            {field: 'partner', title: '易联云用户id'},
        ...
        table.on('tool(pageTable)', function (obj) {
            ...
            if (layEvent === 'edit') {//修改
                ...
                if (res && res.data) {
                    ...
                    $("input[name=apikey]").val(res.data.apikey);
                    $("input[name=machine_code]").val(res.data.machine_code);
                    $("input[name=msign]").val(res.data.msign);
                    $("input[name=partner]").val(res.data.partner);
                    /*diy设置结束*/
