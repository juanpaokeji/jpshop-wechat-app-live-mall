    ...
    layui.use(['table', 'jquery', 'form', 'admin', 'setter', 'element', 'laydate', 'laypage'], function () {
        ...
        function getGroupList(s_info) {
            ...
            arr = {
                ...
                type: 'get',
                params: 'type=1' + s_info + '&is_self=0',
            };
        ...
        $(document).off('click', '.add_write_off').on('click', '.add_write_off', function () {
            ...
            add_write_off_render = getTableRender(arr);//变量名对应 arr 中的 name
        ...
        $(document).off('click', '.count').on('click', '.count', function () {
            ...
            count_render = getTableRender(arr);//变量名对应 arr 中的 name
            layer.open({
                type: 1,
                title: '旗下团员',
                content: count_form,
        ...
        //添加核销员搜索
        form.on('submit(findAWO)', function (data) {//查询
            add_write_off_render.reload({
                where: {searchName: data.field.searchNameAWO},
                page: {curr: 1}
            });
        });

        //查看核销员搜索
        form.on('submit(findSWO)', function (data) {//查询
            pageTableShow.reload({
                where: {searchName: data.field.searchNameSWO},
                page: {curr: 1}
            });
        });

        //旗下团员数量搜索
        form.on('submit(findC)', function (data) {//查询
            count_render.reload({
                where: {searchName: data.field.searchNameC},
                page: {curr: 1}
            });
        });
...
function groupListDiv(info) {
    ...
    return '               <tr>\n' +
        ...
        '                        <td><i class="blue"><a data="' + info.uid + '" href="javascript:void(0)" class="green count">' + info.count + '</a></i></td>\n' +
        '                        <td class="tLeft">\n' +
        '                            <p>总消费佣金：' + sum_money + '</p>\n' +
        '                            <p>待结算佣金：' + on_money + '</p>\n' +
