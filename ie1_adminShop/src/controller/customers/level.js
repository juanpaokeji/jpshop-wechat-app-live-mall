 ...
 * Created DateTime: 应该创建于 2019/6/12
 * js 团长等级
 */
...
layui.define(function (exports) {
    ...
    layui.use(['jquery', 'setter', 'admin', 'table', 'form'], function () {
        ...
        //页面不同属性
        var ajax_method = 'merchantLeaderLevel';//新ajax需要的参数 method
        var cols = [//加载的表格
            {field: 'name', title: '等级名称'},
            {field: 'min_exp', title: '等级经验'},
            {field: 'reward_ratio', title: '奖励比例(%)'},
        ...
        form.on('submit(sub)', function () {
            ...
            arr = {
                ...
                data: {
                    ...
                    min_exp: $('input[name=min_exp]').val(),
                    reward_ratio: $('input[name=reward_ratio]').val(),
                    type: 2,
                    status: status
                }
            };
        ...
        table.on('tool(pageTable)', function (obj) {
            ...
            if (layEvent === 'edit') {//修改
                ...
                if (res && res.data) {
                    ...
                    $("input[name=min_exp]").val(res.data.min_exp);
                    $("input[name=reward_ratio]").val(res.data.reward_ratio);
