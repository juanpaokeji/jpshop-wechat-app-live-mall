...
layui.define(function (exports) {
    layui.use(['jquery', 'form', 'admin', 'setter', 'upload', 'laydate'], function () {
        ...
        var goods_type = sessionStorage.getItem('goods_type');//1代表该商品为门店商品 0 为普通商品
        var arr;//全局ajax请求参数
        var videoData = {};
        //如果类型为新增，则
        ...
        //加载商品分组
        $.ajax({
            url: baseUrl + '/merchantCategoryTypeMini' + key,
            type: "get",
            headers: headers,
            async: false,
            beforeSend: function () {
        ...
        //加载区域分组
        arr = {
            method: 'merchantGoodsCityGroup',//获取所有的省市区
            type: 'get',
            params: 'status=1',
        };
        ...
        if (res && res.data) {
            ...
            for (var a = 0; a < res.data.length; a++) {
                name = res.data[a].name;
                id = res.data[a].id;
                $('select[name=storehouse_id]').append("<option value=" + id + ">" + name + "</option>");
            }
            form.render();
        }
        ...
        // //加载自提点分组
        // arr = {
        //     method: 'merchantTuanUser',//获取所有的省市区
        //     type: 'get',
        //     data: {type: 1, is_self: 1},
        // };
        // var res = getAjaxReturnKey(arr);
        // if (res && res.data) {
        //     var addr;
        //     var id;
        //     $('select[name=band_self_leader_id]').append("<option value='0'>未选择</option>");
        //     for (var a = 0; a < res.data.length; a++) {
        //         addr = res.data[a].addr;
        //         id = res.data[a].id;
        //         $('select[name=band_self_leader_id]').append("<option value=" + id + ">" + addr + "</option>");
        //     }
        //     form.render();
        // }

        //判断是否开启团购，如果未开启，则隐藏 团长佣金和自提点佣金 merchantTuanConfig
        arr = {
            method: 'merchantTuanConfig',//获取团购信息
            type: 'get'
        };
        var res = getAjaxReturnKey(arr);
        if (res && res.data) {
            var is_open = res.data.is_open;
            if (is_open == '1') {
                $('.is_open').show();
            } else {
                $('.is_open').remove();
...
//规格设置结束
        ...
        //获取最低重量
        $(document).on('change', '.stock_weight', function () {
            //循环获取价格框，取最低存入下方最低价格
            var weightArr = [];
            $('input[name=stock_weight]').each(function () {
                if (this.value != '') {
                    weightArr.push(this.value);
                }
            });
            weightArr.sort(sortNumber);//数组排序，取第一个值（最低值）
            $('#weight').val(weightArr[0]);
        });
        ...
        if (goods_id) {
            ...
            $.ajax({
                    ...
                    success: function (res) {
                        ...
                        $('select[name=storehouse_id]').val(data.storehouse_id);
                        ...
                        $("input[name=weight]").val(parseFloat(data.weight));
                        ...
                        if (goods_type === '1') {
                            $('.buttons').empty();
                            if (data.is_check === '0') {
                                $('.buttons').append('<div class="layui-col-md12 save">\n' +
                                    '                <button type="button" class="layui-btn layui-btn-primary" data="1" lay-submit lay-filter="sub">通过</button>\n' +
                                    '                <button type="button" class="layui-btn layui-btn-primary" data="0" lay-submit lay-filter="sub">不通过</button>\n' +
                                    '            </div>');
                            }
                        }
...
//执行添加或编辑
        form.on('submit(sub_goods)', function () {
            ...
            if (property1 == '' && property2 == '') {
                ...
                $("input[name='stock_weight']").each(function (j, item) {
                    if (item.value !== '0') {
                        stock_weight.push(item.value);
                    }
                });
            ...
            subData = {
                ...
                storehouse_id: $('select[name=storehouse_id]').val(),
        ...
        form.on('submit(sub)', function (e) {
            ...
            arr = {
                method: 'merchantGoodAudit/' + goods_id,
                type: 'put',
                params: 'audit=3',
                data: {
                    is_check: is_check
...
//新增规格明细方法
function new_specifications(property1_name, property2_name, stock_price, stock_weight, stock_number, stock_code, stock_cost_price, pic_url, stock_assemble_price) {
    if (!property1_name) {
        property1_name = '';
    }
    if (!property2_name) {
    ...
    return '<div class="layui-form-item specification_details" style="display: flex;flex-direction: row;justify-content: flex-start;">\n' +
        ...
        '                    <div class="layui-input-inline" style="width: 150px;">\n' +
        '                        <span class="unified_style">\n' +
        '                            <span class="asterisk">*</span>重量(kg)\n' +
        '                        </span>\n' +
        '                        <input name="stock_weight" value="' + stock_weight + '" lay-verify="required|number" required\n' +
        '                               class="specification_color1 stock_weight">\n' +
        '                    </div>\n' +
        '                    <div class="layui-input-inline" style="width: 150px;">\n' +
        '                        <span class="unified_style">\n' +
        '                            库存：\n' +
