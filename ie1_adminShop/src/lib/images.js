...
layui.define(function (exports) {
    ...
    layui.use(['jquery', 'setter', 'admin', 'table', 'form', 'laypage'], function () {
        ...
        var ajax_method_images_group = 'pictureGroup';
        ...
        function getImages(picture_group_id) {
            arr = {
                method: 'picture/' + picture_group_id,
                type: 'get',
                data: {
                    limit: limit,
                    page: tabPage
        ...
        $(document).off('click', '.images_delete_icon').on('click', '.images_delete_icon', function () {
            ...
            layer.confirm('确定要删除这张图片么?', function (index) {
                layer.close(index);
                arr = {
                    method: 'merchantGoodsPicture/' + id,
                    type: 'delete'
                };
        ...
        // //点击上传图片执行事件
        // var pageTableUploadImages = $('.pageTableUploadImages');
        // var upload_images_open_index;
        // form.on('submit(upload_images)', function () {
        //     arr = {
        //         method: ajax_method_images_group,
        //         type: 'get',
        //     };
        //     $('select[name=images_group_id]').empty().append("<option value='0'>未分组</option>");
        //     var res = getAjaxReturnKey(arr);
        //     if (res && res.data) {
        //         var name;
        //         var id;
        ...
        //             id = res.data[a].id;
        //             $('select[name=images_group_id]').append("<option value=" + id + ">" + name + "</option>");
        //         }
        //     }
        //     form.render();
        //     upload_images_open_index = layer.open({
        //         type: 1,
        //         title: '新增',
        //         content: pageTableUploadImages,
        //         shade: 0.1,
        //         offset: '100px',
        //         area: ['630px', '500px'],
        //         cancel: function () {
        //             pageTableUploadImages.hide();
        //         }
        //     })
        // });
        ...
        // var images_upload_width = 0;
        // var images_upload_height = 0;
        // $("#addImgPutImagesUpload").change(function () {//加载图片至img
        //     var file = this.files[0];
        //     images_upload_name = file.name;
        //     if (window.FileReader) {
        //         var reader = new FileReader();
        //         reader.readAsDataURL(file);
        //         reader.onloadend = function (e) {
        //             images_upload_url = e.target.result;
        //             $("#imageImagesUpload").attr("src", e.target.result);
        //         };
        //     }
        ...
        //         img = new Image();
        //         img.onload = function () {
        //             images_upload_width = this.width;
        //             images_upload_height = this.height;
        //         };
        ...
        $("#addImgPutImagesUpload").change(function () {//加载图片至img
            ...
            for (var i = 0; i < files_len; i++) {
                ...
                //下面这个 if 里有个严重弄bug，reader.onloadend 方法有时候会延迟执行，导致图片url未存入数组中 待修复
                if (window.FileReader) {
                    var reader = new FileReader();
                    reader.readAsDataURL(file);
                    reader.onloadend = function (e) {
        ...
        //上传图片执行保存按钮
        // form.on('submit(sub_upload_images)', function () {
        //     arr = {
        //         method: 'merchantGoodsPicture',
        //         type: 'post',
        //         data: {
        //             picture_group_id: $('select[name=images_group_id]').val(),
        //             pic_url: images_upload_url,
        //             name: images_upload_name,
        //             width: images_upload_width,
        //             height: images_upload_height
        ...
        //         var img_position = images_common_div_info.indexOf('<img');//img在字符串中的位置
        //         if (img_position > 0) {
        //             images_common_div_info = images_common_div_info.replace('<img', '<img src="' + res.data.src + '"');
        //         } else {
        //             //没有，则传递过来的字符串不符合标准
        //             layer.msg('需要添加的标签不符合标准，标签中必须含有img', {icon: 1, time: 2000});
        //             return;
        ...
        //         layer.close(upload_images_open_index);
        //         pageTableUploadImages[0].reset();//表单重置
        //         pageTableUploadImages.hide();
        //         layer.close(open_index);
        //         // images_div[0].reset();//表单重置
        //         images_div.hide();
        //     }
        // });
        ...
        form.on('submit(group_manage)', function () {
            ...
            arr = {
                ...
                method: ajax_method_images_group + '?key=' + saa_key,//请求的 api 接口方法和可能携带的参数 key
                cols: [cols]//加载的表格字段
            };
